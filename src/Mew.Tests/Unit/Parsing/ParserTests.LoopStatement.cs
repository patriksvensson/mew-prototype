...
{
    [UsesVerify]
    [ExpectationPath("Parser/LoopStatement")]
    public sealed class Loop
    {
        [Fact]
        [Expectation("Loop")]
        public Task Should_Parse_Loop()
        {
            // Given, When
            var expr = ParserFixture.Parse("loop { foo = 2; }");
        ...
        [Theory]
        [InlineData("loop", "Expected '{'")]
        [InlineData("loop {", "Expected '}' after block")]
        public void Should_Produce_Correct_Diagnostics(string input, string expected)
        {
            ...
            result.Root.Statements[0].ShouldBeOfType<LoopStatement>();
        }
    }
}
